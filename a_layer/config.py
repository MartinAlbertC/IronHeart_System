"""A 层（感知层）全局配置 — IranHeart_System 集成版"""

import os
from pathlib import Path

# 项目根路径（IranHeart_System）
SYSTEM_ROOT = Path(__file__).parent.parent
# A 层目录
A_LAYER_ROOT = Path(__file__).parent
# 模型目录（已迁移至 System 内部）
MODELS_DIR = SYSTEM_ROOT / "models"

# 输出目录
DATA_DIR = str(SYSTEM_ROOT / "data")
OUTPUT_DIR = str(SYSTEM_ROOT / "outputs")
...
# BoTSort 跟踪器配置
TRACKER_CONFIG = str(MODELS_DIR / "botsort_custom.yaml")
...
VAD_MIN_SILENCE_MS = 700  # 连续静音超过此时长认定段落结束（700ms 避免歌词碎片化）
VAD_MAX_SPEECH_MS = 30000  # 最长语音段（毫秒），超长则强制切割

# ASR 参数（SenseVoice — 识别效果优于 paraformer-zh）
ASR_MODEL = "iic/SenseVoiceSmall"
ASR_MODEL_DIR = str(MODELS_DIR / "funasr")  # 本地缓存目录（首次自动下载）

# 声纹参数 - 使用绝对路径指向已有的模型文件
...
# ============= 事件输出配置 =============
EVENT_OUTPUT_FILE = str(Path(OUTPUT_DIR) / "a_events_backup.jsonl")
...
# ============= 日志配置 =============
LOG_LEVEL = "INFO"
LOG_FILE = str(Path(OUTPUT_DIR) / "vision_pipeline.log")
AUDIO_LOG_FILE = str(Path(OUTPUT_DIR) / "audio_pipeline.log")
FEISHU_LOG_FILE = str(Path(OUTPUT_DIR) / "feishu_pipeline.log")

# ============= 调试配置 =============
DEBUG_MODE = False
SAVE_VISUALIZATION = False
VISUALIZATION_OUTPUT = str(Path(OUTPUT_DIR) / "output_with_detections.mp4")
