import { ExecutionTrigger, PlannedAction, FeishuPayload, LLMContext } from './types/core';
...
class ActionPlanner {
   ...
   */
  async plan(trigger: ExecutionTrigger): Promise<PlannedAction> {
   ...
   */
  private async callPlanningLLM(trigger: ExecutionTrigger): Promise<{
    actions: Array<{
   ...
   */
  private buildPlanningPrompt(trigger: ExecutionTrigger): string {
    const lines: string[] = [];
    ...
    lines.push('## 用户画像');
    if (ctx.user_persona.health) lines.push(`健康: ${ctx.user_persona.health}`);
    if (ctx.user_persona.preferences) lines.push(`偏好: ${ctx.user_persona.preferences}`);
    lines.push('');
   ...
   */
  private buildLLMContext(trigger: ExecutionTrigger): LLMContext {
    const ctx = trigger.llm_context;

    const constraints: string[] = [];
    if (ctx.user_persona.health) constraints.push(ctx.user_persona.health);
    if (ctx.user_persona.preferences) constraints.push(ctx.user_persona.preferences);
...
export default ActionPlanner.getInstance();
